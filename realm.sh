...
add_port_range_forward() {
    read -e -p "请输入落地鸡的IP: " ip
    read -e -p "请输入本地中转鸡的起始端口: " start_local_port
    read -e -p "请输入本地中转鸡的截止端口: " end_local_port
    read -e -p "请输入落地鸡的起始端口: " start_remote_port
    read -e -p "请输入落地鸡的截止端口: " end_remote_port

    # 检查两个端口范围的长度是否一致
    local_port_range=$((end_local_port - start_local_port + 1))
    remote_port_range=$((end_remote_port - start_remote_port + 1))
    
    if [ $local_port_range -ne $remote_port_range ]; then
        echo "错误：本地端口范围和远程端口范围必须相同!"
        echo "本地端口范围: $local_port_range, 远程端口范围: $remote_port_range"
        return
    fi

    local remote_port=$start_remote_port
    for ((local_port=$start_local_port; local_port<=$end_local_port; local_port++)); do
        echo "
[[endpoints]]
...
remote = \"$ip:$remote_port\"" >> /root/.realm/config.toml
    ...
    echo "端口段转发规则已添加。本地端口 $start_local_port-$end_local_port 对应远程端口 $start_remote_port-$end_remote_port"
}
